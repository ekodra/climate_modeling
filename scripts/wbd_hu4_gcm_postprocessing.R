...
s3_cmip5_rcp45_daily_pr_preprocessed <- aws.s3::get_bucket(
  bucket = export_bucket,
  prefix = "preprocessed_gcm_data/usgs_wbd_hu4/cmip5_daily_rcp45_pr",
  max = Inf
) %>%
  purrr::map(.f = ~`class<-`(.x, "list")) %>%
  dplyr::bind_rows() 
...
s3_cmip5_rcp45_daily_tas_preprocessed <- aws.s3::get_bucket(
  bucket = export_bucket,
  prefix = "preprocessed_gcm_data/usgs_wbd_hu4/cmip5_daily_rcp45_tas",
  max = Inf
) %>%
  purrr::map(.f = ~`class<-`(.x, "list")) %>%
  dplyr::bind_rows() 
...
for(i in 1:nrow(s3_cmip5_rcp45_daily_pr_preprocessed)){
  print(i)
  tmp[[i]] <- daily_pr_postprocessing(s3_obj = s3_cmip5_rcp45_daily_pr_preprocessed$Key[i])
}

daily_pr_statistics_rcp45 <- purrr::reduce(.x = tmp, .f = rbind)

saveRDS(daily_pr_statistics_rcp45, "cmip5_daily_pr_statistics_rcp45.rds")
...
for(i in 1:nrow(s3_cmip5_rcp45_daily_tas_preprocessed)){
  print(i)
  tmp[[i]] <- daily_tas_postprocessing(s3_obj = s3_cmip5_rcp45_daily_tas_preprocessed$Key[i], 
                                       pr_df = daily_pr_statistics_rcp45)
}

daily_tas_statistics_rcp45 <- purrr::reduce(.x = tmp, .f = rbind)

saveRDS(daily_tas_statistics_rcp45, "cmip5_daily_tas_statistics_rcp45.rds")
